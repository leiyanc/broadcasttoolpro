...
| Email | Sending domain, DKIM, and controlled SES delivery are verified | Ready | Verified domain, DKIM, SPF, DMARC, custom MAIL FROM, and real transactional delivery |
| Email | SES production access and bounce/complaint event handling are operational | Ready | Production access approved in `us-east-1`; real send/delivery plus signed SNS bounce and complaint suppression verified end to end |
...
| Billing | Published prices and server-side entitlements agree | Sandbox webhook verified | Base plans and plan-specific additional-channel Price IDs are server-controlled; verify an end-to-end live addition, prorated invoice, period-end removal, and reversal before deployment approval |
| Billing | Payment, invoices, cancellation, refunds, and failed-payment ownership are defined | Partially ready | Checkout, webhook synchronization, subscription cancellation/resumption, per-channel additions and period-end removals, recurring renewal display, and the 72-hour failed-payment grace lifecycle are implemented; finalize exceptional refund procedures before self-service launch |
...
| Privacy | Retention, deletion, and customer data-request procedure are operational | Partially ready | Product owner approved the initial schedule on September 8, 2026; implement and exercise the post-closure controls in [`DATA_PRIVACY_OPERATIONS.md`](DATA_PRIVACY_OPERATIONS.md) |
| GDPR | European privacy information and transfer controls are ready to publish | **Blocking** | Internal bilingual draft and release checklist exist; business address, provider-transfer verification, EU representative assessment, DPA, and professional legal review remain open in [`GDPR_RELEASE_CHECKLIST.md`](GDPR_RELEASE_CHECKLIST.md) |
...
The same workflow provides the Stage 1 external availability monitor after it
is merged into the default branch. See
