...
| Billing | Payment, invoices, cancellation, refunds, and failed-payment ownership are defined | Partially ready | Checkout and webhook synchronization are implemented; configure and verify the Sandbox endpoint, then finalize Customer Portal, refund, cancellation, and failed-payment procedures before self-service launch |
